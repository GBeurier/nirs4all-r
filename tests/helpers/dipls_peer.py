"""Independent Python n4m C-ABI oracle for target-domain DI-PLS."""
...
def main(request_path: str, response_path: str) -> None:
    ...
    source = np.asarray(request["source"], dtype=np.float64)
    ...
    with pls4all.Config() as config:
        ...
        result = pls4all.di_pls_fit(
            pls4all.Context(), config, source, y, target,
            float(request["di_lambda"]),
        )
