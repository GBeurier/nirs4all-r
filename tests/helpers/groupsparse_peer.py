"""Independent Python n4m C-ABI oracle for post-SIMPLS group shrinkage."""
...
def main(request_path: str, response_path: str) -> None:
    ...
    groups = np.asarray(request["group_assignment"], dtype=np.int32)
    with pls4all.Config() as config:
        ...
        result = pls4all.group_sparse_pls_fit(
            pls4all.Context(), config, train, y, groups,
            float(request["group_lambda"]),
        )
